...
Phase 1 introduces only a minimal foundation: manifests, seed objects, and a local validator.
...
- Deprecated objects should remain addressable and use `status: deprecated` rather than being silently reused for a different object.
...
The validator checks that paper manifests parse, referenced files exist, seed objects validate against `schemas/research-object.schema.json`, and stable IDs match `<kind>.<domain>.<slug>`.
